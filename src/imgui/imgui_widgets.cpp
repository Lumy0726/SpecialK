                ...
                float         scale_max,         ImVec2 frame_size )
{
  ...
  if (frame_size.y == 0.0f)
    frame_size.y = label_size.y + (style.FramePadding.y * 2.0f);
  ...
  const bool hovered =
    false;//ItemHoverable (frame_bb, id);
  ...
  RenderFrame (frame_bb.Min, frame_bb.Max, GetColorU32 (GetStyleColorVec4 (ImGuiCol_FrameBg)), true, style.FrameRounding);
...
/// Special K Mod
    ...
    const ImVec4 col_base    = GetStyleColorVec4 ((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLines        : ImGuiCol_PlotHistogram);
    const ImVec4 col_hovered = GetStyleColorVec4 ((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLinesHovered : ImGuiCol_PlotHistogramHovered);
    ...
    for (int n = 0; n < res_w; n++)
    {
      ...
      const float  v1  = values_getter ( data,
              ( v1_idx + values_offset + 1 ) % values_count );
      ...
      if (plot_type == ImGuiPlotType_Lines)
      {
        window->DrawList->AddLine ( pos0, pos1, v_hovered == v1_idx ? GetColorU32 (col_hovered) : GetColorU32 (col_base) );
      }
      ...
      else if (plot_type == ImGuiPlotType_Histogram)
      {
        ...
        window->DrawList->AddRectFilled ( pos0, pos1, v_hovered == v1_idx ? GetColorU32 (col_hovered) : GetColorU32 (col_base) );
      }
